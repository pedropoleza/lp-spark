...
import { Search, Check, Copy, Lock, ShieldCheck, Ticket, ExternalLink, ArrowLeft, Zap } from "lucide-react";
...
export function CheckoutScreen({ plan }: { plan: PlanId }) {
  ...
  // monta o redirect absoluto pro nosso wizard (funciona em qualquer domínio)
  useEffect(() => {
  ...
  const iframeSrc = checkoutLink(plan, { coupon: applied, redirectUrl });
    ...
    <div className="min-h-dvh bg-ink text-cream">
      ...
      <main className="mx-auto grid max-w-5xl gap-6 px-5 pb-12 sm:px-8 md:grid-cols-[minmax(280px,0.85fr)_1.6fr]">
        ...
        <aside className="space-y-5">
          ...
          {/* cupom (mesma engine de referral) */}
          <div className="rounded-card-lg border border-white/10 bg-white/[0.02] p-4">
            ...
            {applied && (
              <p className="mt-2 rounded-lg border border-accent/30 bg-accent/10 p-2.5 text-[11px] text-cream">
                Cupom <span className="font-mono font-semibold text-accent">{appliedLabel}</span> aplicado. Se não
                entrar sozinho, cole no campo <strong>Cupom</strong> do checkout.
              </p>
            )}
          </div>
        ...
        <section className="flex min-h-[70dvh] flex-col overflow-hidden rounded-card-lg border border-white/10 bg-graphite shadow-plan">
          ...
          <a href={iframeSrc} target="_blank" rel="noopener noreferrer" className="flex shrink-0 items-center gap-1.5 px-4 py-2 text-xs text-muted hover:text-cream">
